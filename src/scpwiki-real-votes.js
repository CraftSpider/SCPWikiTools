...
let pageID, pageName, pair, part, scriptContent, els, el, content;

function popUp(message, width, height, reload) {
	reload = reload || false;
	width = width || 200;
	height = height || 200;
	let body = document.getElementById("realVotes").parentNode;
	let masterDiv = document.getElementById('messagePopup');

	if (masterDiv) {
		body.removeChild(masterDiv);
	}

	let marginH = Math.ceil(parseFloat(width) / 2.00);
	let marginV = Math.ceil(parseFloat(height) / 2.00);
	...
	let contentDivHeight = height - 30; //10px for the closeDiv and 10px for the padding
...
function getVoteData(pageID) {
	let voteData = JSON.parse(getVotes(pageID, pageName).responseText);
	let res = "";
