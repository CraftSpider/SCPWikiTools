...
function getPageRead(page) {
	...
	request.open("POST", "http://ferryfm13.thirteen.axc.nl/scp/misc/callWikidot.php?c=pageread&p=" + page + "&m=" + member);
...
function markPageRead() {
	...
	let request = new XMLHttpRequest();
	request.open("POST", "http://ferryfm13.thirteen.axc.nl/scp/misc/callWikidot.php?c=markread&p=" + page + "&m=" + member);
	request.onload = function(response) {
			return response;
		};
	request.send();
...
function checkReadSCP(page, link) {
	let checkRead = new Promise(function(resolve, reject) {
		let request = getPageRead(page);
		request.onreadystatechange = function() {
			if (request.readyState === 4) {
				resolve(parseInt(request.responseText, 10));
			}
		};
		request.send();
	});
	checkRead.then(function(pageRead) {
		if (pageRead === 1) {
...
function main() {
	...
	if (myAccount) {
		//First, we have to grab the logged in person's username
		let userSpan = document.getElementById('login-status').getElementsByClassName('printuser')[0];

		for (let y = 0; y < userSpan.childNodes.length; y++) {
			if (userSpan.childNodes[y].nodeType === 3) {
				member = userSpan.childNodes[y].nodeValue.trim();
				break;
			}
		}

		//Then, we check for what page we are on, of the pages we want to activate on.
		let scpLinks = document.getElementsByTagName("li");

		if (window.location.href.indexOf("scp-series") !== -1) { //Main series
			for (x = 0; x < scpLinks.length; x++) {
				page = scpLinks[x].innerHTML.match(/(SCP-[0-9]{3,4})/);
			...
			for (x = 0; x < scpLinks.length; x++) {
				page = scpLinks[x].innerHTML.match(/(SCP-[0-9]{3,4}-ARC)/);
			...
			for (x = 0; x < scpLinks.length; x++) {
				page = scpLinks[x].innerHTML.match(/(SCP-[0-9]{3,4}-EX)/);
			...
			checkRead.then(function(pageRead) {
				//console.log("pageRead returned " + pageRead);
				...
				if (pageRead === 0 && getValue("autoMark", 0) === 1) {
					markPageRead();
					pageRead = 1;
				}
				...
				if (pageRead === 0 && getValue("autoMark", 0) === 0) {
					img.addEventListener("click", markPageRead, true);
		...
		//console.log("Can't find logged in user.");
	}
}
